...
export class MunicipalLawsComponent implements OnInit {
  ...
  keys = ["overview", "systemOfAccounting", "budget", "accounts", "assets", "liabilities", "annualReport", "inYearFinancialReporting", "externalAudit", "internalAudit", "specialAudit", "performanceReports", "mediumTermFiscalPlan", "publicDisclosure"];
  messages: {};
  structure: {};
  ...
  slides = [];
  constructor(private commonService: CommonService, private http: HttpClient, private modalService: BsModalService) { }
  ...
  ngOnInit() {
    ...
    this.http.get('/assets/files/municipal-laws.json').subscribe((data: any[]) => {
      this.list = data;
      // this.prepareData();
