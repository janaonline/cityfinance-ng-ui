import {
  ...
  export class DigitOnlyDirective implements OnChanges {
    ...
    onPaste(event: any): void {
      ...
      if (windowData['clipboardData']) {
        // Browser is IE
        pastedInput = windowData['clipboardData'].getData('text');
    ...
    private pasteData(pastedContent: string): void {
      ...
      if (!pasted) {
        if (this.inputElement.setRangeText) {
          const { selectionStart: start, selectionEnd: end }: any = this.inputElement;
          ...
          let windowData: any = window;
          if (typeof windowData['InstallTrigger'] !== 'undefined') {
