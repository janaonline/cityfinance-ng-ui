...
export class BasicComponent implements OnInit {
  ...
  transformResult(result) {
    ...
    for (let i = 0; i < result.length; i++) {
      ...
      if(['Debt', 'Tax'].indexOf(result[i].head_of_account) > -1){ // ignore Debt and Tax account types
        continue;
      }

      let keyCode = result[i].code;
      this.report[keyCode] = { 'code': result[i].code, 'line_item': result[i].line_item, 'head_of_account': result[i].head_of_account };
  ...
  populateCalcFields(result, years){
    ...
    if(this.reportReq.reportGroup == 'Balance Sheet' && this.reportReq.type=='Summary' ){ // BS Summary
    ...
    }else if (this.reportReq.type=='Summary' ){ // IE Summary
        ...
        for (let j = 0; j < years.length; j++) {
          ...
          if(subtractFields && subtractFields.length>0){
            for (let x = 0; x < subtractFields.length; x++) {
              ...
              if(result[subtractFields[x]] && result[subtractFields[x]][years[j]['title']]){
