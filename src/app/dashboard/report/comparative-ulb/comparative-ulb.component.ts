...
export class ComparativeUlbComponent implements OnInit {
  ...
  constructor( private reportService: ReportService, private excelService: ExcelService, private reportHelper: ReportHelperService) { }
  ...
  ngOnInit() {
    ...
    this.reportService.reportResponse.subscribe(res => {
      if(res ){
        ...
        this.headerGroup.yearColspan = this.years.length / this.reportReq.years.length;
      } else{
      ...
      for (let i = 0; i < this.reportReq.ulbList.length; i++) {
        ...
        // if(this.report[item.code] && item.ulb_code && item.budget.length > 0){
          for (let j = 0; j < this.reportReq.years.length; j++) {
            ...
            this.years.push({title: key, caption: ulb.name, state: ulb.state, isComparative: false });

            if(j > 0 && this.reportReq.isComparative){
              const comparativeKey = ulb.code + '_' + this.reportReq.years[j-1] + '_' + this.reportReq.years[j];
              this.years.push({title: comparativeKey, caption: '%', state: 'Comparision', isComparative: true });
            }
          }
        // }
      }
    ...
    for (let i = 0; i < this.reportReq.years.length; i++) {
      ...
      for (let j = 0; j < this.reportReq.ulbList.length; j++) {
          ...
          // if(this.report[item.code] && item.ulb_code && item.budget.length > 0){
            ...
            this.years.push({title: key, caption: ulb2.name, state: ulb2.state, isComparative: false });
  ...
  transformToReportFormat_UlbVSUlb(result: any[]) {
    ...
    for (let i = 0; i < resultKeys.length; i++) {
      ...
      if(['Debt', 'Tax'].indexOf(result[resultKeys[i]].head_of_account) > -1){ // ignore Debt and Tax account types
        continue;
      }
      ...
      for(let j = 1; j < this.reportReq.ulbList.length; j++){
        ...
        for (let k = 0; k < this.reportReq.years.length; k++) {
          const keyCode = ulb1.code + '_' + ulb2.code + '_' + this.reportReq.years[k];
    ...
    for (let i = 0; i < resultKeys.length; i++) {
      ...
      for(let j = 0; j < this.reportReq.ulbList.length; j++){
        ...
        for (let k = 1; k < this.reportReq.years.length; k++) {
          const keyCode = ulb.code + '_' + this.reportReq.years[k-1] + '_' + this.reportReq.years[k];
