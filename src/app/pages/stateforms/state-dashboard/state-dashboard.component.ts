...
export class StateDashboardComponent extends BaseComponent implements OnInit {
  ...
  gaugeChart1() {
    ...
    this.gauge_provisional = new Chart(ctx, {
      ...
      data: {
        labels: [
          "Completed and Accounts Submitted",
  ...
  getwaterSuppyData() {
    ...
    for (let i = 0; i < uasList.length; i++) {
      ...
      this._WaterSupplyService.getslbsData(uasList[i]._id).subscribe(
        (res) => {
          let data = res["data"];
          this.statusData = [];
          ...
          if (this.getData[i] != "null") {
            console.log(
              ...
              i,
              this.totalULBsInUA,
              data[1]?.completedAndpendingSubmission.length +
                data[1]?.pendingCompletion.length +
                data[1]?.underStateReview.length +
                data[0]?.total
            );
            this.totalULBsInUA[i] =
              data[1]?.completedAndpendingSubmission.length +
              data[1]?.pendingCompletion.length +
              data[1]?.underStateReview.length +
              data[0]?.total;
