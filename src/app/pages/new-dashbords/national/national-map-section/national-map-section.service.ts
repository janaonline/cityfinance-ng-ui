...
export class NationalMapSectionService {
  ...
  getNationalData(nationalInput) {
    return this.http.get(
      environment.api.url +
        `dashboard/national/data-availability?financialYear=${nationalInput.financialYear}&stateId=${nationalInput.stateId}&population=${nationalInput.populationCat}&ulbType=${nationalInput.ulbType}`
    );
  }
  ...
  DownloadNationalTableData(nationalInput) {
    return this.http.get(
      environment.api.url +
        `dashboard/national/data-availability?financialYear=${nationalInput.financialYear}&stateId=${nationalInput.stateId}&population=${nationalInput.populationCat}&ulbType=${nationalInput.ulbType}&csv=${nationalInput.csv}`,
      { responseType: "blob" }
    );
  }
